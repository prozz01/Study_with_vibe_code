let sections = JSON.parse(localStorage.getItem("fluidSections") || "null") || DEFAULT_SECTIONS;
...
function setEditable(on){
  ...
  $("#saveStatus").textContent=on?"본문을 직접 수정한 뒤 ‘편집 저장’을 누르세요":"자동 저장 준비";
}
function saveEdits(){
  sections.forEach(s=>{
...
$("#exportBtn").onclick=()=>{
  ...
  const data={version:1,sections,completed,exportedAt:new Date().toISOString()};
...
$("#resetBtn").onclick=()=>{
  if(confirm("수정 내용과 학습 완료 기록을 기본 상태로 되돌릴까요?")){
    ...
    sections=JSON.parse(JSON.stringify(DEFAULT_SECTIONS)); completed={}; render(); observe(); toast("초기화했습니다.");
  }
};
...
function showQuiz(){
  if(qi>=quizOrder.length){
    ...
    $("#quizFeedback").textContent=score>=8?"핵심 개념이 잘 연결되어 있습니다.":"틀린 개념을 본문 검색으로 다시 확인해보세요.";
